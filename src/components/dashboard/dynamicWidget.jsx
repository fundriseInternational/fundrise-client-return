    ...
    <div className="absoluteDynamicWidget">
      {widgetState.type === "avatar" && (
        <div className="avatarSection">
          <span type="button" onClick={handlewidgetClose}>
            <i className="icofont-close-line"></i>
          </span>
          <h2>Select Avatar</h2>
          <div className="avatars">
            <button
              className="unitAvatar"
              onClick={() => {
                setCurrentUser({ ...currentUser, avatar: "avatar_1" });
              }}
            >
              <span></span>
            </button>
            <button
              className="unitAvatar"
              onClick={() => {
                setCurrentUser({ ...currentUser, avatar: "avatar_2" });
              }}
            >
              <span></span>
            </button>
          </div>
          <button type="button" onClick={handlewidgetClose}>
            Select
          </button>
        </div>
      )}
      {widgetState.type === "invest" && (
        <div className="avatarSection investwidgetSection">
          ...
          <p>
            You are about to invest in the <span>{investData?.plan}</span>{" "}
            package which takes a period of{" "}
            <span>{investData?.duration} days</span>
          </p>
          <div className="investMinmax">
            {investData?.plan === "Silver"
              ? "Min. Capital: EUR 100 | Max. Capital: EUR 900"
              : investData?.plan === "Gold"
              ? "Min. Capital: EUR 1000 | Max. Capital: EUR 9000"
              : "Min. Capital: EUR 10,000 | Max. Capital: EUR 100,000"}
          </div>
      ...
      {widgetState.type === "withdraw" && totalCapital >= 100 && (
        <div className="avatarSection investwidgetSection">
          ...
          <p>
            You are about to widthraw <span>${withdrawData?.amount}</span> from
            your account.
          </p>
          <div className="investMinmax">
            {"Min. Withdrawable Amount: EUR 200"}
          </div>
          <form className="widgetInvestForm" onSubmit={handleProceedWithdraw}>
            ...
            <div className="unitInputField">
              ...
              <select
                required
                value={withdrawData?.paymentOption}
                onChange={(e) => {
                  setWithdrawData({
                    ...withdrawData,
                    paymentOption: e.target.value,
            ...
            <div className="unitInputField">
              <label htmlFor="name">Wallet Address/Bank Account Number</label>
              <input
                type="text"
                required
      ...
      {widgetState.type === "withdraw" && totalCapital < 100 && (
        <div className="avatarSection emptySesction">
          ...
          <h2>
            Your Balance is insufficient to make a widthrawal at the moment.
            Kindly invest or make a deposit.
          </h2>
        </div>
      )}
      {widgetState.type === "delete" && totalCapital < 100 && (
        <div className="avatarSection investwidgetSection">
          ...
          <i
            ...
            style={{ fontSize: "4em", color: "#DC1262" }}
          ></i>
          <h2>Are you sure you want to delete this account?</h2>
          <div className="bottomBtnCntn">
            <button type="submit" onClick={handleAccoutDelete}>
              Proceed
            </button>
            <button type="button" onClick={handlewidgetClose}>
              Cancel
